...
function resolveManifestKey(
  ...
  const norm = dep.replace(/^\.\/+/, "").replace(/^\/+/, "");
  const exact = manifest.find((m) => m.filePath === norm);
  if (exact) return exact.docKey;

  const stripExt = (p: string) => p.replace(/\.[a-zA-Z0-9]+$/, "");
  const depStripped = stripExt(norm);

  const endsMatch = manifest.find(
    (m) => m.filePath.endsWith("/" + norm) || m.filePath === norm
  );
  if (endsMatch) return endsMatch.docKey;

  const byStripped = manifest.find(
    (m) => stripExt(m.filePath) === depStripped || stripExt(m.filePath).endsWith("/" + depStripped)
  );
  if (byStripped) return byStripped.docKey;
